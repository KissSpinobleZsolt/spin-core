      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-700">
            <th className="text-left pb-2 pr-4 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Prop</th>
            <th className="text-left pb-2 pr-4 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Type</th>
            <th className="text-left pb-2 pr-4 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Default</th>
            <th className="text-left pb-2 font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Description</th>
          </tr>
        </thead>
          ...
          {props.map(p => (
            <tr key={p.name}>
              <td className="py-2 pr-4 font-mono text-blue-600 dark:text-blue-400 whitespace-nowrap">
                {p.name}
                {p.required && <span className="ml-1 text-red-500 dark:text-red-400">*</span>}
              </td>
    ...
    <div id={doc.name.toLowerCase()} className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
      ...
      <div className="px-5 py-4 bg-white dark:bg-slate-800">
        {doc.props.length > 0 ? (
          ...
          <p className="text-xs text-slate-400">No props.</p>
        )}
        {doc.notes && (
