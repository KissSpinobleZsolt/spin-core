...
# Fallback model when the bot record's model field is empty.
...
class OpenAICompatProvider(LLMProvider):
    ...
    Reads connection details from environment variables on instantiation:

    - ``OPENAI_API_KEY`` — required; the API key for authentication.
    - ``OPENAI_BASE_URL`` — optional; override to point at a different base URL
      (e.g. a Groq, Mistral, or local vLLM endpoint).  Defaults to the official
      OpenAI endpoint when not set.

    If either the ``openai`` package is absent or the API key is missing, the
    ``stream`` method raises a ``RuntimeError`` that the chat route forwards to
    the client as a JSON error chunk.
    """
    ...
    async def stream(
        ...
        Args:
            model: Model identifier recognised by the target endpoint, e.g.
                ``"gpt-4o"``, ``"mistral-large-latest"``, or ``"llama-3.3-70b-versatile"``
                for Groq.  Defaults to ``gpt-4o`` when empty.
            messages: Conversation history as ``{"role": str, "content": str}``
                dicts.  The OpenAI API accepts ``system`` as a valid role, so no
                extraction step is needed — the list is passed through unchanged.
            timeout: Request timeout in seconds forwarded to the SDK's HTTP client.

        Yields:
            One `NormalizedChunk` per SSE delta; the final sentinel chunk has
            ``done=True`` and carries token counts from the usage block (only
            present when ``stream_options={"include_usage": True}`` is set,
            which this adapter does automatically).

        Raises:
            RuntimeError: When the ``openai`` package is not installed, the API
                key is missing, or the endpoint returns an error.
        """
