...
# ---------------------------------------------------------------------------
# Audit-column mixin — added to every table for provenance tracking.
#   owner      — email of the current responsible party; 'system' for seeded rows
#   created_by — immutable: email of whoever first created the row ('system' for seeds)
#   updated_by — email of whoever last edited the row; NULL until first explicit edit
#   created_on — server-set creation timestamp
#   updated_on — NULL initially; written only on explicit edits (not at creation)
# ---------------------------------------------------------------------------
...
class UserRow(Base):
    ...
    # audit
    owner      = Column(String, nullable=False, default="system")                         # current responsible party; 'system' for seed/auto-created rows
    created_by = Column(String, nullable=False, default="system")                         # immutable creator identity
    updated_by = Column(String, nullable=True)                                            # identity of last editor; NULL until first explicit update
    created_on = Column(DateTime, server_default=func.now())                              # server-set creation timestamp
    updated_on = Column(DateTime, nullable=True)                                          # NULL until first explicit edit; not set at creation
...
class BotTypeRow(Base):
    ...
    context_strategy = Column(String, nullable=False, default="conversational")              # how conversation history is managed
    # audit
    owner      = Column(String, nullable=False, default="system")
    created_by = Column(String, nullable=False, default="system")
    updated_by = Column(String, nullable=True)
...
class BotRow(Base):
    ...
    # audit — created_on / updated_on renamed from created_at / updated_at via pg_migrations
    owner      = Column(String, nullable=False, default="system")                        # current responsible party
    created_by = Column(String, nullable=True, default="")                               # email of the admin who created this bot
    updated_by = Column(String, nullable=True)                                           # email of the admin who last edited this bot
    created_on = Column(DateTime, server_default=func.now())                             # server-set creation timestamp
    updated_on = Column(DateTime, nullable=True, onupdate=func.now())                    # NULL until first edit; auto-bumped on subsequent writes
...
class TranslationRow(Base):
    ...
    # audit — updated_on renamed from updated_at via pg_migrations
    owner      = Column(String, nullable=False, default="system")
    created_by = Column(String, nullable=False, default="system")
    updated_by = Column(String, nullable=True)
    created_on = Column(DateTime, server_default=func.now())
    updated_on = Column(DateTime, nullable=True)         # written on every merge; replaces the old updated_at
...
class ModuleRow(Base):
    ...
    configuration_raw = Column(JSON, nullable=True, default=None)                             # snapshot of the full manifest.json as received from the module server
    # audit
    owner      = Column(String, nullable=False, default="system")
    created_by = Column(String, nullable=False, default="system")
    updated_by = Column(String, nullable=True)
